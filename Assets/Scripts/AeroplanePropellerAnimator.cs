...
//    [SerializeField] [Range(0f, 11000f)] private float m_ThrottleBlurStart = 0.25f;   // The point at which the blurred textures start.
//    [SerializeField] [Range(0f, 11000f)] private float m_ThrottleBlurEnd = 0.5f;      // The point at which the blurred textures stop changing.
...
//        m_PropellorBlur.parent = m_PropellorModel;
    ...
    private void Update()
    {
		for (int propellerIndex = 0; propellerIndex<4;propellerIndex++) {
	        // Rotate the propellor model at a rate proportional to the throttle.
	//        m_PropellorModel.Rotate(0, propellers.getRPM(propellerIndex)*Time.deltaTime*k_RpmToDps, 0);
			m_PropellorModel[propellerIndex].Rotate(0, 0.001f*propellers.getRPM(propellerIndex)*Time.deltaTime*k_RpmToDps, 0);
		}
		...
		if (propellers.getRPM(propellerIndex) > m_ThrottleBlurStart)
        {
			var throttleBlurProportion = Mathf.InverseLerp(m_ThrottleBlurStart, m_ThrottleBlurEnd, propellers.getRPM(propellerIndex));
